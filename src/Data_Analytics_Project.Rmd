...
def addFieldsToNodes(nodes):
    for element in nodes:
        ...
        if elementId == communities_stats.iloc[0]['max_degree_id']:
...
```{python}
net = readGraph('giantComponent.cyjs')
```
...
```{python}
communities_stats.iloc[0]['max_degree_id']
```
