...
```{python}
categories_stats.iloc[0]
```

```{python}
[data.loc[data["_id"] == cat.iloc[0]["max_degree_id"]] for _,cat in categories_stats.iterrows()]
```

```{python}
[data.loc[data["_id"] == cat["max_degree_id"]] for _,cat in categories_stats.iterrows()]
```
...
if method == "ne":
    for i, row in communities_stats.iterrows():
        titles = data.loc[data['_id'].isin(row["community"])]["title"]
        #print([product_id for product_id in row["community"]])
        row["top_words"]=processTitles(titles)
elif method == "spacy":
    for i, row in communities_stats.iterrows():
        titles = data.loc[data['_id'].isin(row["community"])]["title"]
        #print([product_id for product_id in row["community"]])
        row["top_words"]=processTitlesSpacy(titles)
else:
    for i, row in communities_stats.iterrows():
        merged_titles = " ".join(data.loc[data['_id'].isin(row["community"])]["title"])
        #print([product_id for product_id in row["community"]])
