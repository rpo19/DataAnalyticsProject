...
```{python}
matrix = nx.degree_mixing_matrix(full_graph)
matrix
```
...
def computeStats(data, graph, main_df):
    ...
    max_degree_title = main_df.loc[main_df["_id"] == max_degree_id]["title"]
    # degree
...
if not os.path.isfile("../data/communities_stats_latex.csv"):
    communities_stats[["id", "dominant_category", "cardinality", "max_degree", \
                      "max_degree_title", "avg_clust", "top_words", "top_ents"]]\
    .to_csv("../data/communities_stats_latex.csv", sep="\t", index=False)
```
