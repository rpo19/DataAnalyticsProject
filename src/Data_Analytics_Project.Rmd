...
communities_stats["top_words"] = communities_stats["community"].progress_apply(\
                                        lambda x: processTitlesFun(data.loc[data['_id'].isin(x)]["title"]))
```
...
```{python}
str = "ciao mi chiamo Capitan Harlock"
...
def generateProductsSampleCsv():
    ...
    df.to_csv('../dataApp/sampleProducts.csv', columns=["_id", "title", "category", "price", "avg_rating", "pictures", "questions_number"], sep='\t', index=False)
...
```{python}
cy = nx.readwrite.json_graph.cytoscape_data(full_graph)
cy
```
...
```{python}
prova = convert2cytoscapeJSON(full_graph)
```
...
```{python}
a = json_prova['elements']['nodes'] + json_prova['elements']['edges']
```
...
```{python}
for row in a:
    if row['data']['shared_name'] == 'B06XYPK9M6':
        row['data']['selected'] = True
```

```{python}
nx.nx_agraph.graphviz_layout(full_graph)
```
