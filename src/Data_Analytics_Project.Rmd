...
```{python}
positive_reviews = dataReviewsReduced.loc[dataReviewsReduced['polarity'] == "positive" ] 
```
...
```{python}
positive_reviews["token"]
```

```{python}
negative_reviews = dataReviewsReduced.loc[dataReviewsReduced['polarity'] == "negative"] 
```
...
```{python}
dataReviews["product"].value_counts()[1:5]
```

```{python}
#products_most_reviewed = dataReviews["product"].value_counts()[550:555]
```

```{python}
#most_product_reviews = dataReviews.loc[dataReviews['product'].isin(products_most_reviewed.index)]
```

```{python}
#most_product_reviews.columns
```

```{python}
#len(most_product_reviews)
```

```{python}
#most_product_reviews["cleaned"] = most_product_reviews["body"].progress_apply(lambda x: clean_sentence(x,False))
```
...
```{python}
#prod="B0000AP6LD"
dataReviews_prod = dataReviews[dataReviews["product"]  == prod]
```
...
```{python}
dataReviews_prod["cleaned"] = dataReviews_prod["body"].progress_apply(lambda x: clean_sentence(x,False))
```

```{python}
dataReviews_prod['token']=dataReviews_prod['cleaned'].progress_apply(word_tokenize)
```
...
def compute_LDA(reviews):
    ...
    print("end bigram")
    texts=text_data
    print("start computation of dictionary")
    dictionary = corpora.Dictionary(texts)
    print("end computation of dictionary")
    print("start computation of corpus")
    corpus = [dictionary.doc2bow(text) for text in texts]
    print("end computation of corpus")

    print("start computations of optimal number of topics")
    #num_topics = optimal_topics(dictionary=dictionary, corpus=corpus, texts=texts, max_topic=10)
    print("end computations of optimal number of topics")
    ...
    return ldamodel, corpus, dictionary, LDAvis_prepared
```

```{python}
ris = compute_LDA(dataReviews_prod)
```

```{python}
lda, corpus, dictionary, ldavis= ris
```

```{python}
topics = lda.print_topics()
x = lda.show_topics(num_topics=1, num_words=15, formatted=False)
...
LDAvis_reviews=[]
count=1
for prod in tqdm(products_most_reviewed.index):
    print("LDA for product {}".format(count))
    product_reviews = most_product_reviews[most_product_reviews["product"]  == prod]
    print(prod)
    model, corpus, dictionary, ldavis = compute_LDA(product_reviews)
    lda_models.append(model)
    corpus_reviews.append(corpus)
    dictionary_reviews.append(dictionary)
    LDAvis_reviews.append(ldavis)
    count += 1
```
...
```{python}
len(dataReviews_prod['token'])
```

```{python}
texts=dataReviews_prod['token']
dictionary = corpora.Dictionary(texts) 
corpus = [dictionary.doc2bow(text) for text in texts]
```
...
```{python}
x = ldamodel.show_topics(num_topics=2, num_words=15, formatted=False)
topics_words = [[wd[0] for wd in tp[1]] for tp in x]
```
...
```{python}
dataReviews_prod["token"] 
```

```{python}
text_data = []
for sentence in dataReviews_prod["token"] :
    #print(sentence)
    bigram = list(nltk.bigrams(sentence))
    #print(bigram)
    tokens = []
...
```{python}
topics = ldamodel.print_topics()
x = ldamodel.show_topics(num_topics=3, num_words=15, formatted=False)
topics_words = [[wd[0] for wd in tp[1]] for tp in x]
for sentence in topics_words:
    print(sentence)
```
...
```{python}
pyLDAvis.save_html(LDAvis_prepared, '../data/lda_display' + prod+".html")
