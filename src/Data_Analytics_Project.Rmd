...
```{python}
cytoData = pd.DataFrame(data, columns=["_id", "title", "category", "price", "avg_rating", "reviews_number", "questions_number"])
```

```{python}
cytoData['title'] = cytoData['title'].replace(to_replace='[ \t]+', value=' ', regex=True)
cytoData
```

```{python}
cytoData.to_csv("./cytoProducts.csv", sep="\t", index=False)
```
...
```{python}
cytoEdges = pd.read_csv('cytoEdges.csv', sep="\t")
cytoEdges
```

```{python}
#edges_new.values
cytoEdgesList = list({tuple(sorted(item)) for item in cytoEdges.values})
cytoEdges = pd.DataFrame.from_records(cytoEdgesList, columns=['from', 'to'])
cytoEdges
```

```{python}
cytoEdges = cytoEdges.drop_duplicates(subset=["from", "to"])
cytoEdges.to_csv("./cytoEdges.csv", sep="\t", columns=["from", "to"], index=False)
```
